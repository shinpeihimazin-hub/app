...
### 公式bundledスキル（自作の前に知っておく棚）

全セッションで最初から使えるプロンプトベースのスキル群（`disableBundledSkills` で無効化可）。**フェーズ3.5の再利用探索でまずここを見る**——同じ用途のスキルを自作しない。

| bundledスキル | 用途 |
|---|---|
| `/code-review` | 現在のdiffのバグ・品質レビュー（`ReportFindings` で構造化報告） |
| `/debug` | 不具合の診断 |
| `/batch` | 複数タスクの一括処理 |
| `/loop` | 定期・繰り返し実行（[`17 §3`](./17-claude-code-advanced-operations.md)） |
| `/claude-api` | Claude API の最新モデルID・料金・パラメータのリファレンス |
| `/run` / `/verify` / `/run-skill-generator` | アプリの起動・変更の実挙動確認・その起動レシピの記録 |
| `/init` / `/review` / `/security-review` | CLAUDE.md生成／PRレビュー／セキュリティレビュー（ビルトインコマンドだがSkillツール経由で呼べる） |

一覧は公式 [commands リファレンス](https://code.claude.com/docs/en/commands)（Purpose列が **Skill** のもの）。バージョンで増えるので実装時に確認。
