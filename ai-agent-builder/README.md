...
| MCPサーバー | **Playwright**（実ブラウザ）、**Serena**（LSP意味論コード操作）、**Sequential Thinking**（構造化推論）、**Memory**（永続メモリ→`.claude/agent-memory.json`をgit管理で母艦が育つ）、**MarkItDown**（PDF/Office→MD変換）——いずれも `.mcp.json`・キー不要・検証済み |
| 発見チャネル | **公式プラグインマーケットプレイス**（監査済み255件・全件精査済み、中核5選＋次点を台帳`19` §8に選定済み）、**awesome-claude-code**（36.8k★）、**VoltAgentサブエージェント集**（150+）——台帳`19` §7 |
