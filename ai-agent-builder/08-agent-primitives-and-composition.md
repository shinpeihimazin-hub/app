...
1. **既存プリミティブを探索する**
   ...
   - **実行環境そのものを棚卸しする**: `which <候補>`・`npm ls -g --depth=0`・`pip list`・プリインストール資産（例: `ls /opt/pw-browsers` の実ブラウザ）を確認する。ドキュメント上の候補カタログではなく「いまインストール済みで即使えるもの」を把握する。カタログに載っていても未導入なら、それは選択肢ではなく導入タスクである。
   - **ネットワーク到達性を代表1件で確認する**: 外部ドメインへ実際に到達できるかを検証する（例: `curl -m 5 -sS -o /dev/null -w "%{http_code}" https://<対象ドメイン>`）。マネージド環境ではプロキシのネットワークポリシーが許可リスト外ドメインへのCONNECTを403で遮断することがあり、これは**対象サイト側のアンチボット403と見分けがつかない**。curlで切り分けてから診断する（プロキシ遮断ならどのツールに替えても失敗する＝環境設定の変更が先）。
