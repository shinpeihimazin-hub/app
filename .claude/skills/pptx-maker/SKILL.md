...
```
承認ゲート（①②③）を通らずに次のステップは無い。
裏付けの無い数値を「要確認」マークなしでスライドに書かない。
検証（レンダリング確認 or PowerPointでの目視報告）なしに「完成」と言わない。
```
例外を作れるのはユーザーだけ。「一気に進めて」と明示された場合のみゲートをまとめられる。

## 絶対原則
1. **テンプレ必須。** 会社テンプレ（.pptx）のパスが未指定なら、まず場所を聞く。**ダミーテンプレを勝手に作って進めない。**
2. 事実と推測を混ぜない。裏付けの無い数値・実績は使わず「要確認」としてスライドに明示する。
3. 生成物の品質判定は必ずレンダリング結果（PDF）を自分の目で見て行う。レンダリング不能な環境では「PowerPointでの目視確認をお願いします」と明示し、完成扱いにしない。
4. 機密データはこのセッションの外（Web検索クエリ等）に出さない。

## 合理化封じ表（この思考が浮かんだら停止し、手順に戻る）
| 言い訳 | 現実 |
|---|---|
| 「構成は自明だから承認①は省略」 | 流れの手戻りは全工程で最も高くつく。承認①が最安の保険 |
| 「それらしい数字を入れておけば直してくれる」 | 捏造数値は資料の信頼を破壊する。「要確認」を残すのが仕事 |
| 「deck.jsonができたから完成と言おう」 | 生成物を見ていない完成宣言は嘘。レンダリングか目視依頼まで |
| 「文字が溢れたらフォントを縮めればいい」 | 読めない資料は品質ゼロ。削る（notesへ逃がす） |
| 「テンプレが無いので仮で作って見せよう」 | 実テンプレと違う見た目の確認は無意味。パスを聞け |
